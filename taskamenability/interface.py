...
from taskamenability.save_callback import SaveOnBestTrainingRewardCallback
from taskamenability.vec_monitor import VecMonitor
...
class PPOInterface():
    def __init__(self, x_train, y_train, x_val, y_val, x_holdout, y_holdout, task_predictor, img_shape,
        ...
        self.env = VecMonitor(self.env, self.log_dir)
    ...
    def train(self, num_episodes):
        ...
        callback = SaveOnBestTrainingRewardCallback(check_freq=64, log_dir=self.log_dir)
...
class DDPGInterface():
    def __init__(self, x_train, y_train, x_val, y_val, x_holdout, y_holdout, task_predictor, img_shape,
                 load_models=False, controller_weights_save_path=None, task_predictor_save_path=None,
                 custom_controller=False, actor=None, critic=None, action_input=None, modify_env_params=False,
                 modified_env_params_list=None):
        ...
        self.n_rollout_steps = self.env.controller_batch_size  # number of steps per episode (controller_batch_size + val_set_len) multiply by an integer to do multiple episodes before controller update
        ...
        self.agent = DDPGAgent(nb_actions=n_actions, actor=actor, critic=critic, critic_action_input=action_input,
                               ...
                               batch_size=self.env.controller_batch_size,
                               train_interval=train_interval)

        self.agent.compile(Adam(lr=.01, clipnorm=1.), metrics=['mae'])
    ...
    def save(self, controller_weights_save_path):
